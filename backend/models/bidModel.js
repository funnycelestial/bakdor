...
bidId: {
  ...
  unique: true,
  index: true,
  default: () => `BID_${crypto.randomBytes(4).toString("hex").toUpperCase()}`
},
...
bidSchema.methods.calculateRiskScore = async function() {
  let riskScore = 0;

  // ✅ Check for rapid bidding in the last 60s
  const recentBids = await this.constructor.find({
    'bidder.userId': this.bidder.userId,
    'timing.placedAt': { $gte: new Date(Date.now() - 60000) } // Last 1 min
  });
  ...
  // ✅ Check if the user is relatively new
  const User = mongoose.model('User');
  const user = await User.findById(this.bidder.userId);
...
// Post-save hook to calculate risk score automatically
bidSchema.post("save", async function(doc, next) {
  try {
    // Only calculate risk score for new bids (not updates)
    if (doc.isNew) {
