...
const tokenTransactionSchema = new mongoose.Schema({
  ...
  status: {
    type: String,
    enum: ['pending', 'confirmed', 'failed', 'cancelled', 'success'],
    default: 'pending'
  },
  blockchain: {
...
// Pre-validate middleware to generate transaction ID
tokenTransactionSchema.pre('validate', async function(next) {
...
tokenTransactionSchema.statics.getUserBalanceSummary = async function(userId) {
  const pipeline = [
    { $match: { 'user.userId': new mongoose.Types.ObjectId(userId), status: 'confirmed' } },
    {
      $group: {
        _id: null,
        totalDeposits: {
