...
const rollbackSchema = new mongoose.Schema({
  ...
  customMessage: {
    type: String,
    maxlength: 500 // allow user to explain if "other"
  },
  changedBy: {
...
const auctionSchema = new mongoose.Schema({
 ...
 status: {
  type: String,
  enum: [
    // Draft: Auction created by the user (with or without start time/date).
    // Only visible to the owner. Not visible to other users at all.
    'draft',

    // Pending: Auction has a future start date/time.
    // Visible to everyone (appears in "coming soon" section).
    'pending',

    // Active: Auction is live and ongoing.
    // Visible to everyone.
    'active',

    // Ended: Auction has run its course and finished successfully.
    // Visible to everyone for history / records.
    'ended',

    // Cancelled: Auction was cancelled (by user or admin).
    // Typically visible to the owner, but can also be shown to others if you want a "cancelled" section.
    'cancelled',

    // Suspended: Auction suspended by an admin for violations.
    // Visible only to owner + moderators/admins.
    'suspended'
  ],
  default: 'draft'
},
...
auctionSchema.pre("save", async function(next) {
  if (this.isNew && !this.auctionId) {
    ...
    while (!isUnique && attempts < maxAttempts) {
      ...
      const candidate = `AUC_${random}`;
      
      // Check for uniqueness
      const exists = await this.constructor.findOne({ auctionId: candidate });
      if (!exists) {
        this.auctionId = candidate;
        isUnique = true;
      }
      attempts++;
    }
    ...
    if (!isUnique) {
      ...
      this.auctionId = `AUC_${Date.now().toString(36)}`;
    }
  }
  next();
});
...
auctionSchema.methods.placeBid = function(bidAmount, bidder) {
  if (bidAmount > this.pricing.currentBid) {
    ...
    const existingBidder = this.bidding.uniqueBidders || 0;
    // In a real implementation, you'd check if this bidder already bid
    this.bidding.uniqueBidders = existingBidder + 1;
...
// backend/models/auctionModel.js
auctionSchema.methods.rollbackStatus = function(toStatus, reason, customMessage, userId) {
  const fromStatus = this.status;

  // Prevent rollback if already ended or cancelled
  if (['ended', 'cancelled'].includes(fromStatus)) {
    throw new Error(`Auction is ${fromStatus} and cannot rollback`);
  }
...
auctionSchema.virtual("visibility").get(function () {
  switch (this.status) {
    case "draft":
      // Only owner can see
      return "private";
    ...
    case "cancelled":
      // By default owner-only, but you can make it public if you want
      return "private";

    case "suspended":
      // Owner + moderators/admins
      return "restricted";
